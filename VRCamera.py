...
FULL_REMOTE = True
MY_IP = '192.168.113.219' #'192.168.213.219'
VR_IP = '192.168.113.251' #'192.168.213.156'
ESP   = "192.168.113.157"#'192.168.43.97'
#--------------on ESP32----------------
...
def sendImage():
    ...
    while True:
        try:
            print("sendImage")
            data, address = sockImageReceive.recvfrom(100000)
            #data = data.decode()
            #for element in data convert from radians to degrees
            image = []
            ...
            for i in cv2.imencode('.jpg', img_np)[1]: #only bmp seems to work in godot mobile
                image.append(i)
            #show image
            #print(image)
